...
void RenderManager::startUp(Window& window)
{
	...
	m_lastFrameOcclusionCulledObjects = 0;
	...
	m_lastFrameFlushCandidateCountTime = std::chrono::duration<double, std::milli>{ 0.0 };
	m_lastFrameFlushShadowTime = std::chrono::duration<double, std::milli>{ 0.0 };
	m_lastFrameFlushOccluderPrepassTime = std::chrono::duration<double, std::milli>{ 0.0 };
	m_lastFrameFlushFrustumSetupTime = std::chrono::duration<double, std::milli>{ 0.0 };
	m_lastFrameFlushMainPassTime = std::chrono::duration<double, std::milli>{ 0.0 };
	m_lastFrameFlushCleanupTime = std::chrono::duration<double, std::milli>{ 0.0 };
...
void RenderManager::Flush(const Camera& camera, unsigned int selectedEntityId)
{
	...
	const auto candidateCountStart = std::chrono::high_resolution_clock::now();
	for (std::size_t i = 0; i < m_commandCount; ++i)
	{
		m_lastFrameCommandCount += static_cast<std::size_t>(
			std::max(m_commands[i].mesh->NumBuffers(), 0));
	}
	m_lastFrameFlushCandidateCountTime = std::chrono::high_resolution_clock::now() - candidateCountStart;

	const auto shadowStart = std::chrono::high_resolution_clock::now();
	m_device.RenderShadowMaps(m_commands, m_commandCount, *m_lightingManager);
	m_lastFrameFlushShadowTime = std::chrono::high_resolution_clock::now() - shadowStart;

	if (m_occlusionCullingEnabled)
	{
		m_occlusionCulling.UpdateCamera(
			camera.GetPosition(), camera.GetFacing(), camera.GetProjectionMatrix());
		std::vector<OpenGLGraphicsDevice::OcclusionQueryResult> completedQueries;
		m_device.PollOcclusionQueries(completedQueries);
		for (const OpenGLGraphicsDevice::OcclusionQueryResult& result : completedQueries)
		{
			m_occlusionCulling.ApplyQueryResult(
				result.nodeIndex, result.generation, result.visible);
		}
	}
	m_lastFrameFlushOccluderPrepassTime = std::chrono::duration<double, std::milli>{ 0.0 };

	const auto frustumSetupStart = std::chrono::high_resolution_clock::now();
	const Frustum frustum = Frustum::FromViewProjection(
		camera.GetProjectionMatrix() * camera.GetViewMatrix());
	m_lastFrameFlushFrustumSetupTime = std::chrono::high_resolution_clock::now() - frustumSetupStart;

	const auto mainPassStart = std::chrono::high_resolution_clock::now();
	std::vector<std::uint8_t> occlusionVisibility;
	for (std::size_t i = 0; i < m_commandCount; ++i) 
	{
		...
		else
		{
			...
			std::size_t occlusionCulledSubMeshes = 0;
			const std::vector<std::uint8_t>* occlusionVisibilityPtr = nullptr;
			if (m_occlusionCullingEnabled && m_occlusionMasksEnabled &&
				(selectedEntityId == 0 || command.entityId != selectedEntityId))
			{
				const int bufferCount = (std::max)(command.mesh->NumBuffers(), 0);
				occlusionVisibility.assign(static_cast<std::size_t>(bufferCount), 1);
				bool anyOccluded = false;
				for (int bufferIndex = 0; bufferIndex < bufferCount; ++bufferIndex)
				{
					if (m_occlusionCulling.IsOccluded(command.entityId, bufferIndex))
					{
						occlusionVisibility[static_cast<std::size_t>(bufferIndex)] = 0;
						anyOccluded = true;
					}
				}
				if (anyOccluded) occlusionVisibilityPtr = &occlusionVisibility;
			}

			m_device.DrawCulled(command, camera, *m_lightingManager, frustum,
				visibleSubMeshes, culledSubMeshes, occlusionVisibilityPtr,
				&occlusionCulledSubMeshes);

			m_lastFrameFrustumCulledObjects += culledSubMeshes;
			m_lastFrameOcclusionCulledObjects += occlusionCulledSubMeshes;
			m_lastFrameDrawCallsSaved += culledSubMeshes + occlusionCulledSubMeshes;
		}
	}
	// Reuse the depth generated by the main pass. This avoids a duplicate
	// real-geometry prepass; results are consumed at the start of a later frame.
	if (m_occlusionCullingEnabled && m_occlusionQueriesEnabled &&
		!m_occlusionCulling.CameraMoving() &&
		m_occlusionCulling.ShouldIssueQueries())
	{
		m_occlusionCulling.SelectOccluders(
			camera.GetProjectionMatrix() * camera.GetViewMatrix(),
			m_device.ViewportWidth(), m_device.ViewportHeight());
		const auto queryNodes = m_occlusionCulling.Bvh().SelectQueryNodes(7, 8);
		m_device.IssueOcclusionQueries(m_occlusionCulling.Bvh(), queryNodes,
			m_occlusionCulling.QueryGeneration(), camera);
	}
	m_lastFrameFlushMainPassTime = std::chrono::high_resolution_clock::now() - mainPassStart;

	const auto cleanupStart = std::chrono::high_resolution_clock::now();
	m_commandCount = 0;
	m_lastFrameFlushCleanupTime = std::chrono::high_resolution_clock::now() - cleanupStart;
...
void RenderManager::Loop(FrontEndManager& frontEndManager, FileManager& fileManager, SceneManager& SceneManager, ProjectManager& projectManager, Debug& debug, Input& input, Window& window, EngineState& engineState)
{
...

	{
		...
		if (m_occlusionCullingEnabled)
		{
			m_occlusionCulling.Update(m_commands, m_commandCount);
		}
	}
...
double RenderManager::LastFrameFlushCandidateCountMs() const
{
	return m_lastFrameFlushCandidateCountTime.count();
}

double RenderManager::LastFrameFlushShadowMs() const
{
	return m_lastFrameFlushShadowTime.count();
}

double RenderManager::LastFrameFlushOccluderPrepassMs() const
{
	return m_lastFrameFlushOccluderPrepassTime.count();
}

double RenderManager::LastFrameFlushFrustumSetupMs() const
{
	return m_lastFrameFlushFrustumSetupTime.count();
}

double RenderManager::LastFrameFlushMainPassMs() const
{
	return m_lastFrameFlushMainPassTime.count();
}

double RenderManager::LastFrameFlushCleanupMs() const
{
	return m_lastFrameFlushCleanupTime.count();
}
...
{
	if (m_occlusionCullingEnabled == enabled) return;
	m_occlusionCullingEnabled = enabled;
	// Visibility and transform history cannot cross a disabled interval because
	// the scene may have changed while the system was not observing it.
